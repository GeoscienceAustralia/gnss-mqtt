...
def tcp_handler(bind_ip='0.0.0.0', bind_port=2102, mqtt_host='127.0.0.1', mqtt_port=1883):
    mqtt_client = MqttClient(
        host=mqtt_host, port=mqtt_port, client_id=str(uuid.uuid4()), clean_session=False)
...
if __name__ == '__main__':
    while True:
        try:
            tcp_handler(mqtt_host=sys.argv[1], mqtt_port=int(sys.argv[2]))
